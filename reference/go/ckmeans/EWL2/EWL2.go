...
func fill_even_positions(imin, imax, istep, q int, js []int, S [][]float64, J [][]int, sum_x, sum_x_sq []float64) {
	...
	for i := imin; i <= imax; i += istepx2 {
		...
		S[q][i] = S[q-1][js[r]-1] + dissimilarity(js[r], i, sum_x, sum_x_sq)
		...
		sjimin := dissimilarity(jmax, i, sum_x, sum_x_sq) // ssq(jmax, i, sum_x, sum_x_sq, sum_w)
		r += 1
		for ; r < n && js[r] <= jmax; r++ {
			jabs := js[r]
			...
			s := dissimilarity(jabs, i, sum_x, sum_x_sq)
