...
func TestCKMeansWithNilData(t *testing.T) {
	expected := []Cluster{}
	clusters := CKMeans1dDp(nil, nil, func(v any) float64 { return v.(float64) })
...
func TestCKMeansWithEmptyData(t *testing.T) {
	expected := []Cluster{}
	clusters := CKMeans1dDp([]any{}, nil, func(v any) float64 { return v.(float64) })
...
func TestCKMeansWithInvalidWeights(t *testing.T) {
	...
	CKMeans1dDp([]any{-1.0, 2.0, -1.0, 2.0, 4.0, 5.0, 6.0, -1.0, 2.0, -1.0}, []float64{0.4, 1.3}, func(v any) float64 { return v.(float64) })

	t.Errorf("Expected panic if weights array did not match data array")
}

func TestCKMeans(t *testing.T) {
	x := []any{-0.9, 1.0, 1.1, 1.9, 2.0, 2.1}
	w := []float64{3, 1, 2, 2, 1, 1}
	expected := []Cluster{
		Cluster{Center: -0.9, Variance: 0.0, Values: []any{-0.9}},
		Cluster{Center: 1.06666667, Variance: 0.00666667, Values: []any{1.0, 1.1}},
		Cluster{Center: 1.975, Variance: 0.0275 / 2, Values: []any{1.9, 2.0, 2.1}},
	}

	clusters := CKMeans1dDp(x, w, func(v any) float64 { return v.(float64) })
		...
		for i := range expected {
			...
			if len(clusters[i].Values) != len(expected[i].Values) {
				...
				for j := range expected[i].Values {
					if math.Abs(clusters[i].Values[j].(float64)-expected[i].Values[j].(float64)) > 0.000001 {
						t.Errorf("(cluster %d) invalid 'values' - expected:%v, got:%v", i, expected[i].Values, clusters[i].Values)
					}
				}
			}
...
func TestCKMeansTaps(t *testing.T) {
	...
	expected := []Cluster{
		Cluster{
			Center:   4.52369438,
			Variance: 0.00039172,
			Values:   []any{4.5702720, 4.5061761, 4.5295601, 4.5295601, 4.5178651, 4.4945811, 4.5294081, 4.5236311, 4.5179791, 4.5179111}},
		Cluster{
			Center:   5.05768749,
			Variance: 0.00082230,
			Values:   []any{5.0635940, 5.0459710, 5.0576700, 5.0692840, 5.0227821, 5.1330929, 5.0402341, 5.0402951, 5.0460710, 5.0461651, 5.0694030}},
		Cluster{
			Center:   5.57808420,
			Variance: 0.00042773,
			Values:   []any{5.6035400, 5.5917230, 5.5917220, 5.6034290, 5.5801010, 5.5453951, 5.5627321, 5.5569401, 5.5861020, 5.5510681, 5.5861740}},
		Cluster{
			Center:   6.10048591,
			Variance: 0.00049445,
			Values:   []any{6.1026910, 6.1141730, 6.1374239, 6.1025920, 6.0967150, 6.0677211, 6.0793330, 6.1315849, 6.0909950, 6.0735471, 6.1085690}},
		Cluster{
			Center:   6.61821608,
			Variance: 0.00071530,
			Values:   []any{6.6427089, 6.6191540, 6.6309420, 6.6134550, 6.6541189, 6.5785641, 6.6249740, 6.6541459, 6.5960290, 6.6076360, 6.5786491}},
		Cluster{
			Center:   7.15333449,
			Variance: 0.00045737,
			Values:   []any{7.1417970, 7.1357890, 7.1766839, 7.1476450, 7.1763719, 7.1300970, 7.1416510, 7.1938769, 7.1302250, 7.1650189, 7.1475240}},
		Cluster{
			Center:   7.68575599,
			Variance: 0.00050714,
			Values:   []any{7.7106499, 7.6930719, 7.6989749, 7.6991209, 7.6814059, 7.6524650, 7.6640709, 7.7221128, 7.6525020, 7.6873349, 7.6816069}},
		Cluster{
			Center:   8.21033333,
			Variance: 0.00121729,
			Values:   []any{8.1924709, 8.2038859, 8.2272078, 8.2156099, 8.2155379, 8.1342730, 8.1982709, 8.2445398, 8.1808060, 8.2389538, 8.2621108}},
	}

	clusters := CKMeans1dDp(x, nil, func(v any) float64 { return v.(float64) })
